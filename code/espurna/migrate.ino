...
// 4: based on Embedis, (?) updated module prefixes
...
void migrate() {
    ...
    // Retrieve board name ("friendly name" from 2 tokens, must be the same as HW preprocessor token)
    if (hasSetting("board")) return;

    setSetting("board", MANUFACTURER "_" DEVICE);

    // TODO: check if it is the time to remove this
    if (config_version == 2) {
    ...
    if (config_version == 1) {
        ...
        // TODO: deprecate configuration macros in favour of using code statements directly?
