...
extern "C" void custom_crash_callback(struct rst_info * rst_info, uint32_t stack_start, uint32_t stack_end ) {
    ...
    EEPROMr.put(EepromCrashStart + SAVE_CRASH_CRASH_TIME, crash_time);
    ...
    EEPROMr.write(EepromCrashStart + SAVE_CRASH_RESTART_REASON,
        static_cast<uint8_t>(rst_info->reason));
    EEPROMr.write(EepromCrashStart + SAVE_CRASH_EXCEPTION_CAUSE,
        static_cast<uint8_t>(rst_info->exccause));

    // write epc1, epc2, epc3, excvaddr and depc to EEPROM as uint32_t
    EEPROMr.put(EepromCrashStart + SAVE_CRASH_EPC1, rst_info->epc1);
    EEPROMr.put(EepromCrashStart + SAVE_CRASH_EPC2, rst_info->epc2);
    EEPROMr.put(EepromCrashStart + SAVE_CRASH_EPC3, rst_info->epc3);
    EEPROMr.put(EepromCrashStart + SAVE_CRASH_EXCVADDR, rst_info->excvaddr);
    EEPROMr.put(EepromCrashStart + SAVE_CRASH_DEPC, rst_info->depc);
    ...
    EEPROMr.put(EepromCrashStart + SAVE_CRASH_STACK_START, stack_start);
    EEPROMr.put(EepromCrashStart + SAVE_CRASH_STACK_END, stack_end);
    EEPROMr.put(EepromCrashStart + SAVE_CRASH_STACK_SIZE, stack_size);

    // write stack trace to EEPROM and avoid overwriting settings and reserved data
    // [EEPROM RESERVED SPACE] >>> ... CRASH DATA ... >>> [SETTINGS]
    int eeprom_addr = EepromCrashStart + SAVE_CRASH_STACK_TRACE;
...
void _crashDump(Print& print, bool check) {
    ...
    uint32_t crash_time;
    EEPROMr.get(EepromCrashStart + SAVE_CRASH_CRASH_TIME, crash_time);
    ...
    uint8_t reason = EEPROMr.read(EepromCrashStart + SAVE_CRASH_RESTART_REASON);
    ...
    if (reason == REASON_EXCEPTION_RST) {
        snprintf_P(buffer, sizeof(buffer), PSTR("\nException (%u):\n"),
            EEPROMr.read(EepromCrashStart + SAVE_CRASH_EXCEPTION_CAUSE));
        print.print(buffer);

        uint32_t epc1, epc2, epc3, excvaddr, depc;
        EEPROMr.get(EepromCrashStart + SAVE_CRASH_EPC1, epc1);
        EEPROMr.get(EepromCrashStart + SAVE_CRASH_EPC2, epc2);
        EEPROMr.get(EepromCrashStart + SAVE_CRASH_EPC3, epc3);
        EEPROMr.get(EepromCrashStart + SAVE_CRASH_EXCVADDR, excvaddr);
        EEPROMr.get(EepromCrashStart + SAVE_CRASH_DEPC, depc);
    ...
    EEPROMr.get(EepromCrashStart + SAVE_CRASH_STACK_START, stack_start);
    EEPROMr.get(EepromCrashStart + SAVE_CRASH_STACK_END, stack_end);
    EEPROMr.get(EepromCrashStart + SAVE_CRASH_STACK_SIZE, stack_size);
