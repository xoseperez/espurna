...
#define MQTT_TOPIC_LEDAD ledad

#if MQTT_SUPPORT
void _ledadMQTTCallback(unsigned int type, const char * topic, const char * payload) {

    if (type == MQTT_CONNECT_EVENT) {
        char buffer[strlen(MQTT_TOPIC_LEDAD) + 3];
        snprintf_P(buffer, sizeof(buffer), PSTR("%s/+"), MQTT_TOPIC_LEDAD);
    ...
    if (type == MQTT_MESSAGE_EVENT) {
        ...
        if (!t.startsWith(MQTT_TOPIC_LEDAD)) return;

        // Get led ID
        unsigned int ledID = t.substring(strlen(MQTT_TOPIC_LEDAD)+1).toInt();
        if (ledID >= _ledCount()) {
